...
//Note: The Linux is case sensitive so be aware of specifying correct folder and filename.
#ifdef __IPHONE_4_0
...
char ModelNames[][STRING_LENGTH]= {/*"Sphere1.obj",  "bs_ears.obj","CubeWithNormal.obj",*/ "Plane.obj"};
/*!
...
void ObjLoader::InitModel()
{
    ...
    if (MaterialAmbient >= 0){
        glUniform3f(MaterialAmbient, 0.50f, 0.50f, 0.50f);
    }
    
    if (MaterialSpecular >= 0){
        glUniform3f(MaterialSpecular, 0.6, 0.6, 0.6);
    }
    ...
    if (ShininessFactor >= 0){
        glUniform1f(ShininessFactor, 180.0);
    }
    
    if (LightPosition >= 0){
        glUniform3f(LightPosition, 0.0, 0.0, 5.0 );
    }
...
void ObjLoader::Render()
{
	...
	TransformObj->TransformRotate(180 , 1.0, 0.0, 0.0);
