...
#ifdef __IPHONE_4_0
#define VERTEX_SHADER_PRG			( char * )"TwoSideShadingVertex.glsl"
#define FRAGMENT_SHADER_PRG			( char * )"TwoSideShadingFragment.glsl"
#else
#define VERTEX_SHADER_PRG			( char * )"shader/TwoSideShadingVertex.glsl"
#define FRAGMENT_SHADER_PRG			( char * )"shader/TwoSideShadingFragment.glsl"
#endif
...
char ModelNames[][STRING_LENGTH]= {"SemiHollowCylinder.obj"};
/*!
...
ObjLoader::~ObjLoader()
{
	...
	if ( program = ProgramManagerObj->Program( ( char * )"TwoSidehader" ) )
	{
...
void ObjLoader::InitModel()
{
	...
	if (! ( program = ProgramManagerObj->Program( ( char * )"TwoSidehader" ) ) )
	{
		program = ProgramManagerObj->ProgramInit( ( char * )"TwoSidehader" );
    ...
    char MaterialDiffuseBackFace  = ProgramManagerObj->ProgramGetUniformLocation(program,(char*)"MaterialDiffuseBackFace");
    ...
    if ( MaterialDiffuseBackFace >= 0 ){
        glUniform3f( MaterialDiffuseBackFace, 1.0, 0.0 ,0.0 );
    }

    if ( MaterialDiffuse >= 0 ){
        glm::vec3 color = glm::vec3(0.750, 0.375, 0.0);
