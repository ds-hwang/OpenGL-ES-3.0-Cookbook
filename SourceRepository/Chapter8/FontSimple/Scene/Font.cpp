...
Font::Font(const char* fileName, int Size, Renderer* parent, LanguageType Language)
{
...
void Font::printText(const char* str, GLfloat Red, GLfloat Green, GLfloat Blue, GLfloat Alpha) {
...
void Font::animateText(const char* str, GLfloat Red, GLfloat Green, GLfloat Blue, GLfloat Alpha,float radius,float rot)
{
    // Initialize OpenGL ES States
    glDisable(GL_CULL_FACE);
    glDisable(GL_DEPTH_TEST);
    ...
    for (const char* c = str; *c != '\0'; c++) {
        ...
        theta = 2.0f * PI_VAL * float(a++) / float(num_segments);//get the current angle
        TransformObj->TransformPushMatrix();
        TransformObj->TransformTranslate(radius * cosf(theta), 0.0, radius * sinf(theta));
        const Glyph& gi = glyphs[((unsigned long) *c)];
        TransformObj->TransformRotate(-rot , 0.0, 1.0, 0.0); // adjust
        drawGlyph(gi);
        TransformObj->TransformPopMatrix();
        TransformObj->TransformPopMatrix();
    }
