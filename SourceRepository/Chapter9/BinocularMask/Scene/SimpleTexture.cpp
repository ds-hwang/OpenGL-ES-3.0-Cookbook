...
#ifdef __IPHONE_4_0
#define VERTEX_SHADER_PRG			( char * )"BinocularViewVertex.glsl"
#define FRAGMENT_SHADER_PRG			( char * )"BinocularViewFragment.glsl"
#else
#define VERTEX_SHADER_PRG			( char * )"shader/BinocularViewVertex.glsl"
#define FRAGMENT_SHADER_PRG			( char * )"shader/BinocularViewFragment.glsl"
#endif
...
;
    ...
    X_ASPT_RATIO = ProgramManagerObj->ProgramGetUniformLocation( program, (char *) "horizontalAspectRatio" );
    Y_ASPT_RATIO = ProgramManagerObj->ProgramGetUniformLocation( program, (char *) "verticalAspectRatio" );
    BORDER_COLOR = ProgramManagerObj->ProgramGetUniformLocation( program, (char *) "BorderColor" );
    INNER_RADIUS = ProgramManagerObj->ProgramGetUniformLocation( program, (char *) "LensInnerRadius" );
    OUTER_RADIUS = ProgramManagerObj->ProgramGetUniformLocation( program, (char *) "LensOuterRadius" );

    GLfloat aspectRatio = (GLfloat)RendererHandler->screenWidthPixel() / (GLfloat)RendererHandler->screenHeightPixel();
        if ( RendererHandler->screenWidthPixel() <= RendererHandler->screenHeightPixel() ){
            if(X_ASPT_RATIO>=0){
                glUniform1f(X_ASPT_RATIO, aspectRatio);
            }
            if(Y_ASPT_RATIO>=0){
                glUniform1f(Y_ASPT_RATIO, 1.0);
            }
        }
        else{
            if(X_ASPT_RATIO>=0){
                glUniform1f(X_ASPT_RATIO, 1.0);
            }
            if(Y_ASPT_RATIO>=0){
                glUniform1f(Y_ASPT_RATIO, aspectRatio);
            }
        }
    ...
    if(INNER_RADIUS>=0){
        glUniform1f(INNER_RADIUS, 0.1);
    }
    
    if(OUTER_RADIUS>=0){
        glUniform1f(OUTER_RADIUS, 0.2);
    }
    
    if(BORDER_COLOR>=0){
        glm::vec4 color(1.0, 0.0, 0.0, 0.0);
        glUniform4fv(BORDER_COLOR, 1, (float*)&color);
    }
...
{
    glUseProgram(program->ProgramID);
    float texX = x/(GLfloat)RendererHandler->screenWidthPixel();        // U texture coordinate
    float texY = y/(GLfloat)RendererHandler->screenHeightPixel(); // V texture coordinate
    if(CENTER>=0){
        glm::vec2 center(texX, texY);
        glUniform2fv(CENTER, 1, (float*)&center);
    }
    lastX = x;
    lastY = y;
