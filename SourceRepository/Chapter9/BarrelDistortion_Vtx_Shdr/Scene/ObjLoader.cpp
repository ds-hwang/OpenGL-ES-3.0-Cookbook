...
#ifdef __IPHONE_4_0
#define VERTEX_SHADER_PRG			( char * )"BarrelDistVertex.glsl"
#define FRAGMENT_SHADER_PRG			( char * )"BarrelDistFragment.glsl"
#else
#define VERTEX_SHADER_PRG			( char * )"shader/BarrelDistVertex.glsl"
#define FRAGMENT_SHADER_PRG			( char * )"shader/BarrelDistFragment.glsl"
#endif
...
char ModelNames[][STRING_LENGTH]= {"SemiHollowCylinder.obj","Sphere.obj", "Torus.obj", "Monkey.obj", "IsoSphere.obj", "Cone.obj","CubeWithNormal.obj"};
/*!
...
ObjLoader::~ObjLoader()
{
	...
	if ( program = (ProgramManagerObj->Program( ( char * )"BarrelDistShader" )) )
	{
...
void ObjLoader::InitModel()
{
	...
	if (! ( program = ProgramManagerObj->Program( ( char * )"BarrelDistShader" ) ) )
	{
		program = ProgramManagerObj->ProgramInit( ( char * )"BarrelDistShader" );
    ...
    if ( LightPosition >= 0 ){
        glm::vec3 lightPosition(0.0, 10.0, 10);
    ...
    BARREL_POWER = ProgramManagerObj->ProgramGetUniformLocation( program, (char *) "BarrelPower" );
    glUniform1f(BARREL_POWER, 2.0);
    return;
}
