...
void Camera::AnglesToAxes(glm::vec3 angles)
{
    ...
    // RzRxRy
    // Calculate the LEFT Axis
    Left.x	= cz*cy-sz*sx*sy;
    Left.y	= sz*cy + cz*sx*sy;
    Left.z	= -cx*sy;
...
void Camera::Rotate(glm::vec3 orientation, float angle)
{
    ...
    if(orientation.y == 1.0){
        Forward = glm::normalize( Forward * COS(angle) - Left * SIN(angle));
        Left    = glm::cross( Forward, Up );
    }

    // Rotate along Z axis.
    if(orientation.z == 1.0){
        Left    = glm::normalize( Left * COS(angle) + Up * SIN(angle) );
        Up      -= glm::cross(Forward, Left);
    }
}
...
void Camera::StrafeRightSide ( GLfloat Distance )
{
    ...
    glm::vec3 MoveVector;
    MoveVector.x = Left.x * Distance;
    MoveVector.y = Left.y * Distance;
    MoveVector.z = Left.z * Distance;
    Position += MoveVector;
}
...
{
    glm::vec3 MoveVector;
    MoveVector.x = Up.x * Distance;
    MoveVector.y = Up.y * Distance;
    MoveVector.z = Up.z * Distance;
    Position += MoveVector;
}
